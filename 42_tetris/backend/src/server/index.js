...
const initEngine = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', ({ playerName, roomName, action }) => {
      if (!roomName) {
        if (action === 'join')
          return socket.emit('roomError', 'Room name is required to join.');
      ...
      if (action === 'join' && !games[roomName])
        return socket.emit('roomError', `Room '${roomName}' does not exist.`);
        

      if (action === 'create' && games[roomName])
        return socket.emit('roomError', `Room '${roomName}' already exists.`)
    ...
    socket.on('startGame', ({ roomName }) => {
      ...
      if (game && socket.id) {
        ...
        if (game.players.length < 2)
          return socket.emit('roomError', 'Cannot start game with less than 2 players.');
